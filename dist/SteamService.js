...
const S3Client_1 = require("./dao/S3Client");
class SteamService {
    constructor(bot) {
        this.bot = bot;
        const steamOptions = {};
        if (process.env.S3_BUCKET) {
            ...
            this.client = new steam_user_1.default(steamOptions);
            this.client.storage.on('save', (filename, contents, callback) => {
                (0, S3Client_1.save)(`steam-user/${filename}`, contents)
                    .then(() => callback(null))
                    .catch(err => callback(err));
            });
            this.client.storage.on('read', (filename, callback) => {
                (0, S3Client_1.readFile)(`steam-user/${filename}`)
    ...
    start() {
        ...
        if (sharedSecret) {
            logOnOptions.twoFactorCode = steam_totp_1.default.generateAuthCode(sharedSecret);
        }
        this.client.logOn(logOnOptions);
        this.client.on('loggedOn', () => {
            console.log(`Logged on to Steam as ${this.client.steamID?.getSteamID64()}`);
            this.client.setPersona(steam_user_1.default.EPersonaState.Online);
        ...
        this.client.on('steamGuard', (domain, callback, lastCodeWrong) => {
            ...
            if (sharedSecret) {
                callback(steam_totp_1.default.generateAuthCode(sharedSecret));
            }
            else {
        ...
        try {
            ...
            for (const [tgId, data] of Object.entries(settings)) {
                ...
                for (const steamId of sids) {
                    ...
                    if (this.client.myFriends && this.client.myFriends[steamId] !== steam_user_1.default.EFriendRelationship.Friend) {
    ...
    async handleUserUpdate(steamId, user) {
        ...
        let text = `*${(0, utils_1.escapeMarkdown)(playerName)}* is playing Counter-Strike`;
        if (map || status || score)
            text += `\n`;
        if (map)
            text += `\nMap: ${(0, utils_1.escapeMarkdown)(map)}`;
        if (status)
            text += `\nStatus: ${(0, utils_1.escapeMarkdown)(status)}`;
        if (score)
            text += `\nScore: ${(0, utils_1.escapeMarkdown)(score)}`;
